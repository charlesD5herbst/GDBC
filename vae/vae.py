...
class Encoder(tf.keras.layers.Layer):
    ...
    def __init__(self ,num_units, dim_z , name="encoder" , **kwargs):
        super(Encoder , self).__init__(name=name , **kwargs)

        self.dim_x = (128 , 128 , 1)
        self.dim_z = dim_z
        self.conv_layer1 = tf.keras.layers.Conv2D(filters=num_units , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
        self.conv_layer2 = tf.keras.layers.Conv2D(filters=num_units*2 , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
        self.conv_layer3 = tf.keras.layers.Conv2D(filters=num_units*4 , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
        self.conv_layer4 = tf.keras.layers.Conv2D(filters=num_units*8 , kernel_size=3 , strides=(2 , 2) ,
                                                  padding='same' , activation='relu')
...
class Decoder(tf.keras.layers.Layer):
    ...
    def __init__(self , num_units, dim_z , name="encoder" , **kwargs):
        ...
        self.dim_z = dim_z
        self.dense1 = tf.keras.layers.Dense(units=8 * 8 * num_units*8, activation='relu')
        self.reshape_layer = tf.keras.layers.Reshape((8 , 8, num_units*8))
        self.conv_transpose_layer1 = tf.keras.layers.Conv2DTranspose(filters=num_units*8 , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
        self.conv_transpose_layer2 = tf.keras.layers.Conv2DTranspose(filters=num_units*4 , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
        self.conv_transpose_layer3 = tf.keras.layers.Conv2DTranspose(filters=num_units*2 , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
        self.conv_transpose_layer4 = tf.keras.layers.Conv2DTranspose(filters=num_units , kernel_size=3 , strides=(2 , 2) ,
                                                                     padding='same' , activation='relu')
...
class VAE(tf.keras.Model):
    ...
    def __init__(self ,num_units, dim_z , name="autoencoder" , **kwargs):
        super(VAE , self).__init__(name=name , **kwargs)
        self.dim_x = (128 , 128 , 1)
        self.dim_z = dim_z
        self.encoder = Encoder(num_units,dim_z=self.dim_z)
        self.decoder = Decoder(num_units,dim_z=self.dim_z)
